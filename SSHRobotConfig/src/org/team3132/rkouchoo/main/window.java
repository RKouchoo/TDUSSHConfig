...
public class window {
	...
	public window() {
		...
		try {
			...
			statusReadoutTextBox.setText("[ERR]: Failed to initiate SSH parameters");
			e.printStackTrace();
		}		
		handleButtons();
	}
	...
	private void initializeWindow() {
		...
		lblLocalStatus = new JLabel("Status:");
		lblAuthor = new JLabel("Author: @RKouchoo for Team3132.");
	...
	private void handleButtons() {
		...
		btnConnect.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				// Code for generating keys for testing.
				/* for (int i = 0; i < 20; i ++) {
					jKeyList.add(String.format("%s", i));
				}
				*/
		...
		btnPushToRobot.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				// fancy stuff that has not been done yet
				...
				// Check if the json has been initialized previously contains data.
				if (json == null) {
					statusReadoutTextBox.setText("[ERR]: No data in JSON file!");
					return;
				}
				
				// Parse the json blob to make it pretty in-case of manual editing.
				...
				if (session.isConnected()) {
					try {
						ChannelSftp sftpConnection = (ChannelSftp) session.openChannel("sftp");
						sftpConnection.connect();	
						sftpConnection.put(blobIn, Constants.SSH_REMOTE_FILE + ".save"); // Make a backup copy of the previous config.
						sftpConnection.put(jsonStream, Constants.SSH_REMOTE_FILE + ".test"); // TODO: REMOVE .test WHEN TESTED!
		...
		jKeyList.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent e) {
				// TODO: make sure clicking on it again does not clear the text.
				dataEntryTextBox.setText(jKeyList.getSelectedItem());
			}
		});
	}
}
